...
`appcast.xml` → (with `PUBLISH=1`) publishes both to the web-host site dir behind
...
cert — the same pattern as its sibling apps on web-host. The origin `nginx` listens
...
The public site is probed every 30s by the web-host monitoring stack
