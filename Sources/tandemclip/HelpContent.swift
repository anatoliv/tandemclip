...
enum HelpCatalog {
    ...
    static let topics: [HelpTopic] = [
        ...
        .init(id: "pause", category: "Getting started", title: "Pause",
              body: "Pause stops syncing in both directions. Nothing you copy leaves this Mac, and nothing your other Macs copy arrives here. The menu-bar icon changes to a pause glyph so the state is visible without opening anything, and the menu item itself flips to **Resume**.\n\nTwo things carry on while paused. Your history keeps filling: TandemClip still records what you copy on this Mac; it simply sends none of it. And the AI features keep running, so a long copy can still be given a smart title, which sends that text to your endpoint. If your reason for pausing is that what you are copying should not leave the Mac, privacy hold is the switch you want. See **Pause or privacy hold?**\n\n**Start paused** (Settings → General) makes this the state at every launch, which suits a Mac you only occasionally want syncing.\n\nExample: pause on a laptop you are about to present from, so nothing you copy appears on the Mac in the other room."),
        ...
        .init(id: "semantic-search", category: "Clipboard picker", title: "Search by meaning",
              body: "The picker's search matches more than words: it looks at each clip's full text, the text inside screenshots (recognized on this Mac), and the meaning of what you type, so \"that container command\" finds your docker clip. Everything runs on-device; searching sends nothing anywhere."),
        ...
        .init(id: "services", category: "Clipboard picker", title: "Send from any app",
              body: "Select text or files in any app and use the app menu ▸ Services ▸ \"Send to TandemClip\" (also in the right-click Services submenu) to share it to your Macs immediately, without copying first. Appears after the first launch following an update; log out/in if macOS is slow to pick it up."),
        ...
        .init(id: "privacy-hold", category: "Clipboard picker", title: "Privacy hold ✋",
              body: "The hand button in the picker footer stops anything you copy from leaving this Mac until you switch it off: no broadcasts, no pull serving, no previews, and AI cleanup calls are paused too. Receiving keeps working. The menu-bar menu carries the same switch as **Privacy Hold**, and the icon shows a raised hand while it's on. Deleting a clip still reaches your other Macs while it is on, which is deliberate: a removal gives nothing away and privacy is when you most want it to take effect. Pause is the broader switch and behaves differently; see **Pause or privacy hold?**\n\nExample: turn it on before copying passwords or unreleased numbers, then off when done."),
        ...
        .init(id: "general-name", category: "Settings — General", title: "Display name",
              body: "The name your other Macs see for this computer, shown in the picker's group headers, the peer list, and sync history. Two Macs should not share a name."),
        ...
        .init(id: "sync-max-size", category: "Settings — Sync", title: "Max clipboard size",
              body: "The biggest clip that will sync (default 5 MB, up to 100 MB; large clips travel in chunks automatically, and all Macs need a recent version past ~25 MB). Anything larger falls back to just its plain text, or is skipped if there's no text. Raise it if you often copy large images or files between Macs."),
        ...
        .init(id: "content-storage", category: "Settings — Content", title: "Received-files storage",
              body: "Files received from your Macs are cached on disk so paste keeps working. Past the limit (10 MB to 1 GB, default 200 MB) the oldest clips are evicted automatically. Picking them from history re-materializes them. The clip currently on your clipboard is never evicted."),
        ...
        .init(id: "ai-on-receive", category: "Settings — AI", title: "Automatic AI on clips",
              body: "Two separate opt-ins (off by default, since they send clip text to your endpoint automatically): Smart titles give long clips a short AI-generated name (marked ✨) in the picker; Translate incoming shows a translation in the hover preview when a clip arrives in another language. Detection is on-device, and the clip itself is never altered."),
        ...
        .init(id: "pause-vs-privacy-hold", category: "Private by design", title: "Pause or privacy hold?",
              body: "Both stop your copies leaving this Mac, so they look like two names for one switch. They differ in three ways, and the differences matter.\n\n**Incoming clips.** Pause stops them; privacy hold does not. With privacy hold on, clips from your other Macs still arrive and still land on this clipboard. So if you want to keep receiving while you copy something sensitive here, privacy hold is the one to use.\n\n**AI.** Privacy hold stops it; pause does not. Cleanup, Ask, smart titles and translation of incoming clips all stop talking to your endpoint the moment privacy hold is on. A paused Mac, by contrast, will still give a long copy a smart title, and that sends the text of the copy to your model. This catches people out, so it is worth stating plainly: pausing does not stop your clipboard reaching your AI provider. Privacy hold does.\n\n**Deleting a clip everywhere.** Privacy hold still lets a delete travel to your other Macs. Paused, it cannot. That is deliberate: a delete gives nothing away, because the other Macs already hold the content, and privacy is exactly when you want a removal to take effect.\n\nAs a rule of thumb, pause when you want this Mac out of the conversation for a while, on a shared network or during a presentation. Use privacy hold for the next few copies, when the thing you are about to copy is a password, a key, or a number that is not public yet. For that job privacy hold is both the narrower switch and the safer one.\n\nBoth live in the menu-bar menu, next to each other. Privacy hold is also the ✋ in the picker footer, and the two move together."),
        .init(id: "privacy-passwords", category: "Private by design", title: "Password managers",
              body: "Content a password manager marks as secret is never synced, and the same goes for one-time and transient copies. This relies on the source app setting the marker (1Password, Keychain and most managers do); treat unmanaged secrets with care, or flip on the ✋ privacy hold first."),
        ...
        .init(id: "privacy-quarantine", category: "Private by design", title: "Received files are quarantined",
              body: "Files that arrive from peers are written with restrictive permissions and macOS quarantine, and are revealed in Finder rather than auto-opened, so you decide what runs."),
    ...
    static let releases: [HelpRelease] = [
        .init(version: "0.25.0", date: "September 6, 2026",
              ...
              changes: [
                ...
                .init(.improved, "Help explains Pause and Privacy hold properly, including the part that surprises people: pausing does not stop the AI features, so a paused Mac can still send a long copy to your model for a smart title. Privacy hold is the switch that stops that, and it keeps receiving clips while it does."),
        ...
        .init(version: "0.22.8", date: "July 3, 2026",
              ...
              changes: [
                .init(.improved, "TandemClip is now open source under the MIT license. No user-facing changes in this release, only internal cleanup and project housekeeping."),
              ]),
        ...
        .init(version: "0.22.2", date: "July 3, 2026",
              ...
              changes: [
                .init(.improved, "The hover preview card now has a ? that opens the “Hover previews & quick actions” article, so help is a click away wherever you are in the picker."),
              ]),
        .init(version: "0.22.1", date: "July 3, 2026",
              highlight: "Help, right where you are.",
              changes: [
                .init(.added, "Help buttons in the clipboard picker: a ? in the footer, in compose, and on “Ask your clipboard” opens the matching Help article, so guidance is one click away from the picker too."),
              ]),
        ...
        .init(version: "0.21.3", date: "July 3, 2026",
              ...
              changes: [
                .init(.improved, "Rolled the design system across Settings and the picker, with crisper corners and controls that use the brand color instead of system blue."),
              ]),
        .init(version: "0.21.2", date: "July 3, 2026",
              highlight: "A cleaner, more consistent look.",
              changes: [
                .init(.improved, "Polished the Help window to TandemClip's design system, with consistent type and spacing, crisper cards, and a documented set of shared style tokens."),
              ]),
        ...
        .init(version: "0.21.0", date: "July 3, 2026",
              ...
              changes: [
                .init(.added, "This What's New page, with every version of TandemClip, newest first."),
              ]),
        ...
        .init(version: "0.19.0", date: "July 3, 2026",
              highlight: "Run AI cleanup off your ChatGPT subscription, with no API key needed.",
              changes: [
